...
| `hub.financial.read` | Ver recebíveis, lista «sem cobrança», dashboard financeiro, fluxo de caixa (leitura) |
| `hub.financial.write` | Despesas, ajustes em recebível, **waive** de faturação (motivo obrigatório) |
| `hub.receivables.create` | Ação «Gerar cobrança» (criar `hub_receivables` + linhas); no MVP pode coincidir com `hub.financial.write` nas mesmas roles |
| `hub.cash.session` | Abrir/fechar caixa, sangria/suprimento |
| `hub.cash.receive` | Registar pagamento na recepção (`hub_payments`) |
| `hub.payments.read` | *(Legado doc)* Preferir `hub.financial.read` |
| `hub.payments.write` | *(Legado doc)* Preferir `hub.cash.receive` + `hub.receivables.create` |
...
| Permissão | Descrição |
|-----------|-----------|
| `grooming.queue.read` | Ver fila operacional Banho & Tosa (`/hub/banho-tosa`) |
| `grooming.queue.manage` | Avançar estágio, prioridade, eventos de sessão (API); na **UI Hub**, edição completa do drawer também exige `hub.appointments.write` (ver plano Banho & Tosa). |
| `hub.service_types.write` | (Reutilizado) Na UI da fila B&T, utilizadores **com** esta permissão vêem preços e subtotal de referência no drawer; sem ela, vêem apenas nomes de serviços/adicionais. |
