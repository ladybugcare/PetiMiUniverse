import { expect, test } from '@playwright/test';
import { loginAs } from './helpers/auth';
import { e2eOpsNames, hubCadminCredentials } from './helpers/env';

test.describe('equipe', () => {
  test('CADMIN vê o motorista e2e na lista', async ({ page }) => {
    const creds = hubCadminCredentials();
    test.skip(!creds, 'CADMIN e2e não provisionado.');
    if (!creds) return;

    const { driverName } = e2eOpsNames();
    await loginAs(page, creds.email, creds.password);
    await page.goto('/hub/equipe');

    await expect(page.getByRole('heading', { name: 'Equipe' })).toBeVisible({ timeout: 20_000 });
    await expect(page.getByText('Carregando equipe…')).toBeHidden({ timeout: 20_000 });
    await expect(page.getByRole('heading', { name: 'Erro' })).toHaveCount(0);
    await expect(page.getByText(driverName).first()).toBeVisible();
  });
});
