...
export const createCompositeDemand = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { data: masterDemand, error: demandError } = await supabase
      .from('demands')
      .insert({
        title,
        description,
...
export const getAvailablePositions = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (specialty) {
      query = query.eq('specialty', specialty as string);
    }
...
export const getPositionApplications = async (req: Request, res: Response) => {
...
export const getDemandWithPositions = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (positions && positions.length > 0) {
      for (const position of positions) {
        ...
        (position as any).specialties = specialties?.map(s => s.specialty_name) || [];
      }
    }
...
export const getVetApplications = async (req: Request, res: Response) => {
