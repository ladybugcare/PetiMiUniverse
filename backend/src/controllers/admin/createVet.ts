// backend/controllers/admin/createVet.ts
...
export const createVet = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const authUser = await createAuthUser(email, finalPassword, name, 'vet');
    ...
    try {
      await sendWelcomeEmail(email, name, 'vet', finalPassword, !!generate_password);
    } catch (err) {
