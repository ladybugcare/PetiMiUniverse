// backend/controllers/admin/createFreelancer.ts
...
export const createFreelancer = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const authUser = await createAuthUser(email, finalPassword, name, 'freelancer');
    ...
    try {
      await sendWelcomeEmail(email, name, 'freelancer', finalPassword, !!generate_password);
    } catch (err) {
