...
export type ExamOrderValidationMeta = {
...
function drawExamOrderBody(doc: PdfDoc, snapshot: ExamOrderSnapshot, margin: number, pageW: number): number {
  let y = margin;
...
export async function streamValidatableExamOrderPdf(
  res: Response,
  snapshot: ExamOrderSnapshot,
  validation: ExamOrderValidationMeta,
): Promise<void> {
