...
const GUARDIAN_SELECT =
  'id, clinic_id, full_name, phone, email, notes, created_at, updated_at, deleted_at, client_kind, legal_name, birth_date, sex, tax_id, id_doc_type, id_doc_number, lead_source, postal_code, state, city, district, street, street_number, complement, client_status';

const clientKindSchema = z.enum(['individual', 'company']);
...
const createHubGuardianBodySchema = z.object({
  clinic_id: uuidStr,
...
const updateHubGuardianBodySchema = z.object({
  clinic_id: uuidStr,
...
async function fetchPetsByGuardianIds(
  clinicId: string,
  guardianIds: string[]
): Promise<Map<string, HubGuardianPetRow[]>> {
  const map = new Map<string, HubGuardianPetRow[]>();
  ...
  for (const row of data as unknown as LinkRow[]) {
    ...
    if (row.role !== 'primary' && row.role !== 'secondary') continue;
...
export const getHubGuardianStats = async (req: Request, res: Response) => {
  try {
    ...
    const [
      { count: totalAll, error: e1 },
      { count: activeStatus, error: e2 },
      { count: newMonth, error: e3 },
      petsRes,
    ] = await Promise.all([
      supabaseAdmin
        .from('hub_guardians')
    ...
    const guardianWithPet = new Set<string>();
    if (petsRes.data) {
      for (const pet of petsRes.data as { hub_pet_guardians: { guardian_id: string }[] | null }[]) {
...
export const getHubGuardianById = async (req: Request, res: Response) => {
  try {
...
export const listHubGuardians = async (req: Request, res: Response) => {
  try {
    const parsed = uuidStr.safeParse(req.query.clinic_id);
    if (!parsed.success) {
      return res.status(400).json({ error: 'clinic_id é obrigatório e deve ser UUID' });
...
export const createHubGuardian = async (req: Request, res: Response) => {
  try {
...
export const updateHubGuardian = async (req: Request, res: Response) => {
  try {
    const idParsed = uuidStr.safeParse(req.params.id);
    if (!idParsed.success) {
      return res.status(400).json({ error: 'id inválido' });
    ...
    if (d.client_kind !== undefined) patch.client_kind = d.client_kind;
    if (d.legal_name !== undefined) patch.legal_name = d.legal_name;
    if (d.birth_date !== undefined) patch.birth_date = d.birth_date;
    if (d.sex !== undefined) patch.sex = d.sex;
    if (d.tax_id !== undefined) patch.tax_id = d.tax_id;
    if (d.id_doc_type !== undefined) patch.id_doc_type = d.id_doc_type;
    if (d.id_doc_number !== undefined) patch.id_doc_number = d.id_doc_number;
    if (d.lead_source !== undefined) patch.lead_source = d.lead_source;
    ...
    if (d.district !== undefined) patch.district = d.district;
    if (d.street !== undefined) patch.street = d.street;
    if (d.street_number !== undefined) patch.street_number = d.street_number;
    if (d.complement !== undefined) patch.complement = d.complement;
    if (d.client_status !== undefined) patch.client_status = d.client_status;
