...
const PROSPECT_SELECT =
  'id, clinic_id, full_name, tax_id, phone, email, created_at, updated_at, deleted_at';
...
const patchProspectBodySchema = z
  .object({
    clinic_id: uuidStr,
...
export const listHubProspects = async (req: Request, res: Response) => {
  try {
...
export const getHubProspect = async (req: Request, res: Response) => {
  try {
...
export const createHubProspect = async (req: Request, res: Response) => {
  try {
...
export const patchHubProspect = async (req: Request, res: Response) => {
  try {
    const idParsed = uuidStr.safeParse(req.params.id);
    if (!idParsed.success) {
      return res.status(400).json({ error: 'id inválido' });
    ...
    if (tax_id !== undefined) {
      ...
      if (!n) return res.status(400).json({ error: 'CPF/documento inválido' });
      patch.tax_id = n;
    }
