...
const putGroupAddonsSchema = z
  .object({
    clinic_id: uuidStr,
    addon_service_type_ids: z.array(uuidStr).max(200),
  })
  .strict();

const putAddonAvailabilitySchema = z
  .object({
    clinic_id: uuidStr,
    items: z
      .array(
...
const putAddonDeploymentsSchema = z
  .object({
    clinic_id: uuidStr,
    items: z
      .array(
        z.object({
          service_group_slug: z.string().min(1).max(64),
          enabled: z.boolean(),
        })
      )
      .max(100),
...
const ADDON_SELECT =
  'id, clinic_id, code, name, service_group, cost_amount, sale_amount, pricing_matrix, default_duration_minutes, active, allow_scheduling, description, is_addon';

async function assertAddonType(clinicId: string, addonId: string): Promise<boolean> {
...
async function assertParentService(clinicId: string, parentId: string): Promise<{ service_group: string } | null> {
  const { data } = await supabaseAdmin
    .from('hub_service_types')
    .select('id, clinic_id, service_group, is_addon, deleted_at')
    .eq('id', parentId)
    .maybeSingle();
  if (!data || data.clinic_id !== clinicId || data.deleted_at || data.is_addon) return null;
  return { service_group: String(data.service_group) };
}

async function listParentServicesInGroup(
  clinicId: string,
...
async function isAddonLinkedToGroup(
  clinicId: string,
  serviceGroupSlug: string,
  addonId: string
): Promise<boolean> {
...
async function activateAddonForAllParentServicesInGroup(
  clinicId: string,
  serviceGroupSlug: string,
  addonId: string
): Promise<void> {
  ...
  for (const parent of parents) {
    await supabaseAdmin.from('hub_service_type_addon_availability').upsert(
      {
...
async function clearAddonAvailabilityInGroup(
  clinicId: string,
  serviceGroupSlug: string,
  addonId: string
): Promise<void> {
...
async function ensureAddonInGroup(
  clinicId: string,
  serviceGroupSlug: string,
  addonId: string
): Promise<void> {
  if (await isAddonLinkedToGroup(clinicId, serviceGroupSlug, addonId)) return;
  ...
  const { data: links } = await supabaseAdmin
    ...
    .limit(1);
  const nextOrder =
    links?.length && links[0].sort_order != null ? Number(links[0].sort_order) + 1 : 0;

  const { error } = await supabaseAdmin.from('hub_service_group_addons').insert({
    clinic_id: clinicId,
...
export async function linkAddonToGroup(
  clinicId: string,
  serviceGroupSlug: string,
  addonId: string
): Promise<void> {
...
export async function unlinkAddonFromGroup(
  clinicId: string,
  serviceGroupSlug: string,
  addonId: string
): Promise<void> {
...
export async function seedAddonAvailabilityForNewService(
  clinicId: string,
  ...
  const rows = links.map((l) => ({
    ...
    addon_service_type_id: l.addon_service_type_id as string,
    is_available: true,
  }));
  await supabaseAdmin.from('hub_service_type_addon_availability').upsert(rows, {
    onConflict: 'parent_service_type_id,addon_service_type_id',
  });
}

/** GET /api/hub/service-groups/:id/addons?clinic_id= */
export const getHubServiceGroupAddons = async (req: Request, res: Response) => {
  try {
    ...
    const ids = (links ?? []).map((l) => l.addon_service_type_id as string);
    if (ids.length === 0) {
...
export const putHubServiceGroupAddons = async (req: Request, res: Response) => {
  try {
    const groupId = uuidStr.safeParse(req.params.id);
    if (!groupId.success) return res.status(400).json({ error: 'id inválido' });
    ...
    const slug = group.slug as string;
    ...
    const prevSet = new Set((prevLinks ?? []).map((r) => r.addon_service_type_id as string));
    ...
    if (uniqueIds.length > 0) {
      ...
      const { error: insErr } = await supabaseAdmin.from('hub_service_group_addons').insert(rows);
      if (insErr) {
...
export const getHubServiceTypeAddonAvailability = async (req: Request, res: Response) => {
  try {
    ...
    if (!parent) return res.status(404).json({ error: 'Serviço não encontrado' });

    const { data: groupAddons } = await supabaseAdmin
      .from('hub_service_group_addons')
      .select('addon_service_type_id, sort_order')
      .eq('clinic_id', clinicId.data)
    ...
    const addonIds = (groupAddons ?? []).map((g) => g.addon_service_type_id as string);
    ...
    const availMap = new Map(
      (avail ?? []).map((a) => [a.addon_service_type_id as string, Boolean(a.is_available)])
    );
...
export const putHubServiceTypeAddonAvailability = async (req: Request, res: Response) => {
  try {
    const parentId = uuidStr.safeParse(req.params.id);
    if (!parentId.success) return res.status(400).json({ error: 'id inválido' });
    ...
    const { data: groupAddons } = await supabaseAdmin
      ...
      .eq('clinic_id', clinic_id)
      .eq('service_group_slug', parent.service_group);
    const allowed = new Set((groupAddons ?? []).map((g) => g.addon_service_type_id as string));
    ...
    return getHubServiceTypeAddonAvailability(
      { ...req, params: { id: parentId.data }, query: { clinic_id } } as unknown as Request,
      res
    );
  } catch (e) {
...
export async function resyncAddonAvailabilityOnGroupChange(
  clinicId: string,
  parentServiceTypeId: string,
  newGroupSlug: string
): Promise<void> {
  ...
  for (const row of existingAvail ?? []) {
    const aid = row.addon_service_type_id as string;
    if (!allowed.has(aid)) {
      await supabaseAdmin
        .from('hub_service_type_addon_availability')
        .delete()
  ...
  for (const addonId of allowed) {
    const { data: row } = await supabaseAdmin
      ...
      .maybeSingle();
    if (!row) {
      await supabaseAdmin.from('hub_service_type_addon_availability').insert({
        parent_service_type_id: parentServiceTypeId,
        addon_service_type_id: addonId,
        is_available: false,
      });
...
export const getHubAddonDeployments = async (req: Request, res: Response) => {
  try {
    ...
    const inGroupSlugs = new Set((groupLinks ?? []).map((l) => l.service_group_slug as string));
    ...
    const deploymentGroups = await Promise.all(
      (groups ?? []).map(async (g) => {
        ...
        const services: Array<{ id: string; name: string; is_available: boolean }> = [];
        ...
        if (in_group && parents.length > 0) {
          ...
          const availMap = new Map(
            (avail ?? []).map((a) => [
              a.parent_service_type_id as string,
...
export const putHubAddonDeployments = async (req: Request, res: Response) => {
  try {
    const addonId = uuidStr.safeParse(req.params.id);
    if (!addonId.success) return res.status(400).json({ error: 'id inválido' });
    ...
    return getHubAddonDeployments(
      { ...req, params: { id: addonId.data }, query: { clinic_id } } as unknown as Request,
      res
    );
  } catch (e) {
...
export const getHubServiceTypeAvailableAddons = async (req: Request, res: Response) => {
  try {
    const parentId = uuidStr.safeParse(req.params.id);
    const clinicId = uuidStr.safeParse(req.query.clinic_id);
    if (!parentId.success || !clinicId.success) {
