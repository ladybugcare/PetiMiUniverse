...
import {
  ...
  findEligiblePackageBalance,
  loadPackageWithItems,
  newPurchaseGroupId,
  redeemPackageBalancesOnCheckout,
  reversePackageRedemptionsForComanda,
  eligibleBalancesForComandaItem,
  autoApplyEligiblePackagesToItems,
  pricingVariantsEqual,
} from './hubPackagesService';
import { createSaleStockOut } from './hubInventoryStockUtils';
import type { HubQuotePricingVariantInput } from './hubPricingResolve';

const uuidStr = z.string().uuid();

async function resolveComandaItemPricingVariant(item: Record<string, unknown>): Promise<HubQuotePricingVariantInput | null> {
  const originType = String(item.origin_type ?? '');
  const originId = item.origin_id as string | null;
  if (originType === 'appointment_service' && originId) {
    ...
    if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
      return raw as HubQuotePricingVariantInput;
    }
  }
  return null;
}
...
import {
  ...
  editScopeErrorMessage,
  type ComandaEditContext,
  type ComandaEditScopes,
} from './comandaEditScopes';

export type { ComandaEditScopes } from './comandaEditScopes';

const comandaEditContextSchema = z.enum(['caixa', 'financeiro']).optional().default('caixa');

async function assertComandaEditAllowed(
  comandaId: string,
  clinicId: string,
  context: ComandaEditContext
): Promise<void> {
  ...
  if (!comandaRow) throw new Error('NOT_FOUND');

  const { balance_due } = await computeComandaBalancePayload(comandaId, clinicId);
  const operational_complete = await isOperationalCompleteForComanda(comandaRow as Record<string, unknown>);
  const scopes = computeComandaEditScopes(comandaRow as Record<string, unknown>, operational_complete, balance_due);
  if (!scopes[context]) {
...
async function isOperationalCompleteForComanda(comanda: Record<string, unknown>): Promise<boolean> {
  ...
  if (originType === 'package') return true;
...
export async function tryAutoCloseComanda(comandaId: string, clinicId: string): Promise<boolean> {
  const { data: comandaRow, error } = await supabaseAdmin
    ...
    .eq('clinic_id', clinicId)
    .maybeSingle();
  if (error || !comandaRow) return false;
  if (String(comandaRow.status) !== 'aberta') return false;
  ...
  if (comandaFresh) {
    await refreshComandaFinancialStatus(comandaId, clinicId, comandaFresh as Record<string, unknown>);
  }
  return true;
}

async function syncAndTryAutoCloseComanda(clinicId: string, comandaId: string): Promise<void> {
  try {
...
function comandaItemInsertRow(
  clinicId: string,
  comandaId: string,
  it: Omit<ComandaItemInsert, 'clinic_id' | 'comanda_id'>,
  ...
  if (it.package_balance_id != null) row.package_balance_id = it.package_balance_id;
  return row;
}
...
async function buildComandaItemsFromAppointment(
  ...
  const applied = await autoApplyEligiblePackagesToItems({
    clinicId,
    guardianId,
    items: built.items,
    resolvePricingVariant: async (item) => {
      if (item.origin_type === 'appointment_service' && item.origin_id) {
        const { data: svc } = await supabaseAdmin
          .from('hub_appointment_services')
          .select('pricing_variant')
          ...
          .maybeSingle();
        const raw = svc?.pricing_variant;
        if (raw && typeof raw === 'object') return raw as HubQuotePricingVariantInput;
      }
      return null;
    },
    skipIfAlreadyCovered: async (item) => {
      // Fatura de série (fase 2): se a migration existir e o appointment_service estiver coberto.
      if (item.origin_type !== 'appointment_service' || !item.origin_id) return false;
      const { data, error } = await supabaseAdmin
        .from('hub_series_invoice_items')
        .select('id, invoice:hub_series_invoices(status)')
        .eq('appointment_service_id', item.origin_id)
        .limit(5);
      if (error) return false;
      for (const row of data ?? []) {
        const inv = row.invoice as { status?: string } | { status?: string }[] | null;
  ...
  return {
    items: built.items,
    subtotal: applied.appliedCount > 0 ? applied.subtotal : built.subtotal,
...
async function buildComandaItemsFromGroomingSession(
    ...
    resolvePricingVariant: async (item) => {
      if (item.origin_type === 'appointment_service' && item.origin_id) {
        const { data: svcRow } = await supabaseAdmin
          ...
          .eq('id', item.origin_id)
          .maybeSingle();
        const raw = svcRow?.pricing_variant;
        if (raw && typeof raw === 'object') return raw as HubQuotePricingVariantInput;
      }
      return null;
    },
  ...
  return {
    items: allItems,
    subtotal: applied.appliedCount > 0 ? applied.subtotal : round2(svc.subtotal + extras.subtotal),
...
const openComandaBodySchema = z
  .object({
    ...
    pet_ids: z.array(uuidStr).min(1).max(20).optional(),
    package_lines: z
      .array(
        z
          .object({
  ...
  .superRefine((data, ctx) => {
    if (data.origin_type === 'manual') {
      ...
      if (!data.guardian_id) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'guardian_id obrigatório para venda de pacote' });
      }
...
export const postHubComandaOpen = async (req: Request, res: Response) => {
  try {
    ...
    const effectiveOriginId =
      origin_type === 'manual'
        ? (b.origin_id ?? randomUUID())
        : origin_type === 'package'
          ? (b.origin_id ?? newPurchaseGroupId())
          : (b.origin_id as string);
    ...
    if (origin_type === 'manual') {
      ...
      const contextPetId = b.pet_id ?? null;
      const items: Omit<ComandaItemInsert, 'clinic_id' | 'comanda_id'>[] = [];
      let subtotal = 0;
      let sort = 0;
      ...
      const pkgBuilt = await buildComandaItemsFromPackagePurchase({
        clinicId: clinic_id,
        packageId: b.package_id as string,
        purchaseGroupId: effectiveOriginId,
        petId: b.pet_id ?? null,
        petIds: b.pet_ids,
        packageLines: b.package_lines?.map((ln) => ({ packageId: ln.package_id, petId: ln.pet_id })),
        guardianId: b.guardian_id as string,
    ...
    if (msg === 'PACKAGE_INACTIVE') return res.status(409).json({ error: 'Pacote inativo' });
...
async function listAllowedGuardiansForPetIds(petIds: string[]): Promise<AllowedGuardianRow[]> {
  ...
  const byId = new Map<string, AllowedGuardianRow>();
  for (const row of links ?? []) {
    const gRaw = (row as Record<string, unknown>).hub_guardians;
    const g = (Array.isArray(gRaw) ? gRaw[0] : gRaw) as Record<string, unknown> | null;
    if (!g || g.deleted_at) continue;
    const gid = g.id as string;
    const role = String((row as Record<string, unknown>).role ?? '');
    const existing = byId.get(gid);
    if (existing) {
      if (role === 'primary' && existing.role !== 'primary') existing.role = 'primary';
      continue;
    }
    byId.set(gid, {
      id: gid,
...
async function getHubComandaDetailPayload(comandaId: string, clinicId: string) {
  ...
  const petIds = [
    ...new Set(
      [
        ...(items ?? []).map((it) => it.pet_id as string | null),
        petId,
  ...
  const guardian = guRes.data
    ? {
        id: (guRes.data as Record<string, unknown>).id,
  ...
  const packageBalancesByItemId: Record<string, unknown[]> = {};
  if (
    guardianId &&
    String(comandaRow.origin_type) !== 'package' &&
    String(comandaRow.origin_type) !== 'series_invoice'
  ) {
    for (const it of enrichedItems) {
      if (!(openItemIds as string[]).includes(it.id as string)) continue;
      if (String(it.item_kind) !== 'service' || !it.hub_service_type_id) continue;
      if (String(it.origin_type) === 'appointment_service' && it.origin_id) {
        const { data: covered } = await supabaseAdmin
          .from('hub_series_invoice_items')
          .select('id, invoice:hub_series_invoices(status)')
          .eq('appointment_service_id', it.origin_id as string)
          .limit(5);
        const active = (covered ?? []).some((row) => {
          const inv = row.invoice as { status?: string } | { status?: string }[] | null;
          const status = Array.isArray(inv) ? inv[0]?.status : inv?.status;
      ...
      const lineVariant = await resolveComandaItemPricingVariant(it as Record<string, unknown>);
      const eligible = await eligibleBalancesForComandaItem({
        clinicId,
        guardianId,
        petId: (it.pet_id as string | null) ?? null,
        hubServiceTypeId: it.hub_service_type_id as string,
        pricingVariant: lineVariant,
      });
      if (eligible.length) packageBalancesByItemId[it.id as string] = eligible;
    }
  }
...
async function loadComandaPdfPayload(comandaId: string, clinicId: string) {
  ...
  return {
    ...
    closed_at: (comanda.closed_at as string | null) ?? null,
    ...
    items: detail.items.map((it) => ({
      ...
      pet_name: (it as Record<string, unknown>).pet_name as string | null | undefined,
    })),
...
export const ensureComandaPublicToken = async (req: Request, res: Response) => {
  try {
    ...
    if (!existing || existing.deleted_at) return res.status(404).json({ error: 'Comanda não encontrada' });
...
export const getPublicComanda = async (req: Request, res: Response) => {
  try {
    ...
    if (!comanda || comanda.deleted_at) return res.status(404).json({ error: 'Comanda não encontrada' });
...
export const getHubComandaPdf = async (req: Request, res: Response) => {
  try {
    const idParsed = uuidStr.safeParse(req.params.id);
    const clinicParsed = uuidStr.safeParse(req.query.clinic_id);
    if (!idParsed.success || !clinicParsed.success) {
...
const applyPackageBodySchema = z
  .object({
    clinic_id: uuidStr,
    item_id: uuidStr,
...
export const postHubComandaApplyPackage = async (req: Request, res: Response) => {
  try {
    const idParsed = uuidStr.safeParse(req.params.id);
    ...
    const detail = await getHubComandaDetailPayload(comandaId, clinic_id);
    const item = (detail.items as Record<string, unknown>[]).find((it) => it.id === item_id);
    if (!item) return res.status(404).json({ error: 'Item não encontrado' });
    if ((detail.invoiced_item_ids as string[]).includes(item_id)) {
      return res.status(409).json({ error: 'Item já faturado' });
    }

    const comanda = detail.comanda as Record<string, unknown>;
    const guardianId = comanda.guardian_id as string;
    const petId = (item.pet_id as string | null) ?? null;
    const serviceTypeId = item.hub_service_type_id as string | null;
    ...
    if (package_balance_id) {
      if (!serviceTypeId) return res.status(400).json({ error: 'Somente linhas de serviço podem usar pacote' });
      if (String(item.origin_type) === 'appointment_service' && item.origin_id) {
        const { data: covered } = await supabaseAdmin
          .from('hub_series_invoice_items')
          .select('id, invoice:hub_series_invoices(status)')
          .eq('appointment_service_id', item.origin_id as string)
          .limit(5);
        const active = (covered ?? []).some((row) => {
          const inv = row.invoice as { status?: string } | { status?: string }[] | null;
          const status = Array.isArray(inv) ? inv[0]?.status : inv?.status;
        ...
        if (active) {
          return res.status(409).json({
            error: 'Este serviço já está coberto por fatura em série e não pode usar pacote',
          });
        }
      }
      const lineVariant = await resolveComandaItemPricingVariant(item as Record<string, unknown>);
      ...
      const balVariant = (balance.pricing_variant as HubQuotePricingVariantInput | null) ?? null;
      if (balVariant && !pricingVariantsEqual(balVariant, lineVariant)) {
      ...
      const { data: svc } = await supabaseAdmin
        ...
        .eq('id', item.origin_id as string)
        .maybeSingle();
      if (svc) {
        unitAmount = round2(Number(svc.sale_amount_applied ?? 0));
        lineTotal = unitAmount;
        const st = svc.hub_service_types as { name?: string } | { name?: string }[] | null;
        const name = Array.isArray(st) ? st[0]?.name : st?.name;
        description = (name as string) || description.split(' — Pacote')[0].trim();
      }
    ...
    const openItems = (detail.items as Record<string, unknown>[])
      .filter((it) => !(detail.invoiced_item_ids as string[]).includes(it.id as string))
    ...
    const msg = (e as Error)?.message;
    if (msg === 'NOT_FOUND') return res.status(404).json({ error: 'Comanda não encontrada' });
...
export const postHubComandaCheckout = async (req: Request, res: Response) => {
  try {
    ...
    if (action === 'cancel') {
      ...
      await reversePackageRedemptionsForComanda(clinic_id, comandaId);
    ...
    const rollbackReceivables = async () => {
      for (const mid of stockMovementIds) {
        await supabaseAdmin.from('hub_stock_movements').delete().eq('id', mid);
      }
    ...
    for (const it of items) {
      ...
      const stockResult = await createSaleStockOut({
        ...
        lotId: invLotId,
        qty,
        notes: `Venda na comanda ${comandaId}: ${String(it.description ?? 'Produto')}`,
        referenceType: 'hub_comanda_item',
        referenceId: it.id as string,
        createdBy: userId,
      });
      ...
      if (!stockResult.skipped) {
        stockMovementIds.push(stockResult.id);
      }
    }
    ...
    if (receivableIds.length === 0) {
      ...
      if (hasPackageCoverage && (action === 'receive_now' || action === 'leave_pending')) {
        await redeemPackageBalancesOnCheckout({
          clinicId: clinic_id,
          comandaId,
          staffUserId: userId,
        });
    ...
    if (String(comanda.origin_type) === 'package' && comanda.origin_id) {
      ...
      if (packagePurchaseLines.length) {
        await fulfillPackagePurchasesOnComandaCheckout({
          clinicId: clinic_id,
          comandaId,
          purchaseGroupId: comanda.origin_id as string,
          guardianId,
...
async function applySyncComandaFromOrigin(
  ...
  const discount = Number(comanda.discount_amount ?? 0);
  const insertRows = desiredFiltered.map((it) => comandaItemInsertRow(clinicId, comandaId, {
    ...it,
    sort_order: sortBase++,
  }));
...
export async function syncOpenComandasAfterAppointmentOperationalComplete(
  clinicId: string,
  appointmentId: string
): Promise<void> {
...
export async function maybeFlagComandaCancellationPending(
  ...
  try {
    ...
    const { data: existing, error: exErr } = await supabaseAdmin
      .from('hub_comandas')
      .select('id, cancellation_pending_at, cancellation_resolved_at, unit_id, guardian_id')
      .eq('id', comandaId)
      .eq('clinic_id', clinicId)
      .is('deleted_at', null)
      .maybeSingle();
    ...
    const isNewPending = !existing.cancellation_pending_at;
    if (isNewPending) {
    ...
    // Só avisa quando a pendência nasce, para não repetir o aviso a cada sincronização.
    if (isNewPending) {
      void notifyHubCancellationPending({
        clinicId,
        unitId: (existing.unit_id as string | null) ?? null,
        comandaId,
        guardianId: (existing.guardian_id as string | null) ?? null,
...
/** Comandas abertas ligadas diretamente a agendamentos (origin_type = appointment). */
export async function openComandaIdsForAppointments(
  clinicId: string,
  ...
  const { data, error } = await supabaseAdmin
    ...
    .eq('status', 'aberta')
    .is('deleted_at', null)
    .in('origin_id', appointmentIds);
  if (error) throw new Error(error.message);
  for (const row of data ?? []) {
    const oid = row.origin_id as string | null;
    if (oid) out.set(oid, row.id as string);
  }
  return out;
}
...
async function enrichCancellationQueueRows(
  ...
  const headerPetIds = [...new Set(rows.map((r) => r.pet_id).filter(Boolean))] as string[];
  const comandaIds = rows.map((r) => r.id as string);

  const [guRes, itemsRes, enrichedByComanda] = await Promise.all([
    guIds.length
    ...
    comandaIds.length
      ? supabaseAdmin
          .from('hub_comanda_items')
          .select('comanda_id, pet_id, description, sort_order')
    ...
    Promise.all(
      comandaIds.map(async (cid) => {
        ...
        let { paid_total, balance_due } = await computeComandaBalancePayload(cid, clinicId);
        // Cura: recebível pago sem auto-close deixava comanda aberta “fantasma” no perfil.
        if (String(row.status) === 'aberta' && balance_due <= 0.02) {
          try {
            const closed = await tryAutoCloseComanda(cid, clinicId);
            if (closed) {
              row.status = 'fechada';
              ({ paid_total, balance_due } = await computeComandaBalancePayload(cid, clinicId));
            }
          } catch (e) {
            console.error('enrichComandaRows tryAutoCloseComanda', cid, e);
          }
        }
  ...
  const itemsByComanda = new Map<string, Array<{ pet_id: string | null; description: string }>>();
  const itemPetIds = new Set<string>();
  for (const it of itemsRes.data ?? []) {
    const cid = it.comanda_id as string;
    const petId = (it.pet_id as string | null) ?? null;
    if (petId) itemPetIds.add(petId);
  ...
  const allPetIds = [...new Set([...headerPetIds, ...itemPetIds])];
  const petRes = allPetIds.length
    ? await supabaseAdmin.from('hub_pets').select('id, name').in('id', allPetIds)
    : { data: [] as Record<string, unknown>[] };

  const guMap = new Map((guRes.data ?? []).map((g) => [g.id as string, g]));
  const petMap = new Map((petRes.data ?? []).map((p) => [p.id as string, p]));
  type ComandaEnrichment = {
    paid_total: number;
    balance_due: number;
    operational_complete: boolean;
    edit_scopes: ReturnType<typeof computeComandaEditScopes> | null;
  };
  const enrichedMap = new Map<string, ComandaEnrichment>(enrichedByComanda as [string, ComandaEnrichment][]);

  return rows.map((r) => {
    const cid = r.id as string;
    const gu = r.guardian_id ? guMap.get(r.guardian_id as string) : null;
    const headerPetId = r.pet_id as string | null;
    const headerPet = headerPetId ? petMap.get(headerPetId) : null;
    const items = itemsByComanda.get(cid) ?? [];

    const itemLabels: string[] = [];
    const seenLabel = new Set<string>();
    for (const it of items) {
      const desc = it.description;
      if (!desc || seenLabel.has(desc)) continue;
      seenLabel.add(desc);
      itemLabels.push(desc);
    }

    const petsById = new Map<string, { id: string; name: string }>();
    if (headerPet) {
      petsById.set(headerPet.id as string, { id: headerPet.id as string, name: String(headerPet.name ?? '') });
    }
    for (const it of items) {
      if (!it.pet_id) continue;
      const p = petMap.get(it.pet_id);
      if (!p || petsById.has(it.pet_id)) continue;
      petsById.set(it.pet_id, { id: it.pet_id, name: String(p.name ?? '') });
    }
    const pets = [...petsById.values()].sort((a, b) => a.name.localeCompare(b.name, 'pt-BR'));

    const extra = enrichedMap.get(cid);
    return {
      ...r,
      guardian: gu ? { id: gu.id, full_name: gu.full_name } : null,
      pet: headerPet ? { id: headerPet.id, name: headerPet.name } : pets[0] ?? null,
      pets,
...
export const postHubComandaAddItems = async (req: Request, res: Response) => {
  try {
    ...
    const defaultPetId = (comanda.pet_id as string | null) ?? null;
    const insertRows = items.map((it) => ({
      clinic_id,
      comanda_id: comandaId,
      pet_id: it.pet_id ?? defaultPetId,
  ...
  try {
    ...
    if (guardian_id !== undefined) {
      ...
      const petIds = [
        ...new Set(
          (currentDetail.items ?? [])
            .map((it) => it.pet_id as string | null)
            .filter(Boolean),
        ),
      ] as string[];
      if (petIds.length === 0) {
