...
export type SpecialistReferralValidationMeta = {
...
function drawReferralBody(doc: PdfDoc, snapshot: SpecialistReferralSnapshot, margin: number, pageW: number): number {
  let y = margin;
  ...
  drawField(
    ...
    `${snapshot.veterinarian.full_name}${crmv ? ` · CRMV ${crmv}` : ''}`,
    margin,
    y,
    pageW,
  );
...
export async function streamValidatableSpecialistReferralPdf(
  res: Response,
  snapshot: SpecialistReferralSnapshot,
  validation: SpecialistReferralValidationMeta,
): Promise<void> {
  ...
  if (qrBuffer) doc.image(qrBuffer, margin, y, { width: qrSize, height: qrSize });
