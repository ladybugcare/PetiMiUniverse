...
const basisSchema = z.enum(['percent_of_sale', 'fixed_per_sale']);
...
export const listHubCommissionRules = async (req: Request, res: Response) => {
  try {
...
export const postHubCommissionRule = async (req: Request, res: Response) => {
  try {
...
export const patchHubCommissionRule = async (req: Request, res: Response) => {
  try {
    ...
    const { data: rule, error: rErr } = await supabaseAdmin
      ...
      .is('deleted_at', null)
      .maybeSingle();
    if (rErr || !rule || rule.clinic_id !== clinic_id) return res.status(404).json({ error: 'Regra não encontrada' });

    const nextBasis = basis ?? (rule.basis as string);
    ...
    if (rate !== undefined) patch.rate = nextRate;
...
const deleteRuleQuerySchema = z
  .object({
    clinic_id: uuidStr,
  })
  .strict();

export const deleteHubCommissionRule = async (req: Request, res: Response) => {
  try {
    const idParsed = uuidStr.safeParse(req.params.id);
...
export const getHubCommissionPreview = async (req: Request, res: Response) => {
  try {
