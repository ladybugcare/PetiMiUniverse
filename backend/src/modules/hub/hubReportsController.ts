 ...
 */
export const getHubAbsentClientsReport = async (req: Request, res: Response) => {
  try {
    ...
    if (unitRaw && !unitParsed?.success) return res.status(400).json({ error: 'unit_id inválido' });
    const unit_id = unitParsed?.success ? unitParsed.data : null;

    const daysParsed = z.coerce.number().int().min(7).max(730).safeParse(req.query.days);
    ...
    const lastActivity = new Map<string, { at: string; source: 'appointment' | 'receivable' }>();
    ...
    let apptQ = supabaseAdmin
      ...
      .limit(8000);
    if (unit_id) apptQ = apptQ.or(`unit_id.eq.${unit_id},unit_id.is.null`);
    const { data: appts, error: aErr } = await apptQ;
    if (aErr) return res.status(500).json({ error: aErr.message });
    for (const row of appts ?? []) {
      const gid = row.guardian_id as string;
      if (!gid || lastActivity.has(gid)) continue;
      lastActivity.set(gid, { at: row.starts_at as string, source: 'appointment' });
    }
    ...
    let recQ = supabaseAdmin
      ...
      .limit(8000);
    if (unit_id) recQ = recQ.or(`unit_id.eq.${unit_id},unit_id.is.null`);
    ...
    for (const g of guardians ?? []) {
      const act = lastActivity.get(g.id as string);
      if (act && act.at >= cutoffIso) continue;
    ...
    items.sort((a, b) => {
      if (a.never_attended !== b.never_attended) return a.never_attended ? 1 : -1;
 ...
 */
export const getHubNoShowsReport = async (req: Request, res: Response) => {
  try {
    const clinic = uuidStr.safeParse(req.query.clinic_id);
    if (!clinic.success) return res.status(400).json({ error: 'clinic_id inválido' });
    const clinic_id = clinic.data;
    ...
    let boardQ = supabaseAdmin
      .from('hub_boarding_reservations')
      .select(
        `id, mode, status, expected_check_in, expected_check_out, pet_id, guardian_id, unit_id,
      ...
      .limit(500);
    if (unit_id) boardQ = boardQ.eq('unit_id', unit_id);
    ...
    const guIds = [
      ...new Set(
        [
          ...(boardingRows ?? []).map((r) => r.guardian_id as string | null),
          ...(apptRows ?? []).map((r) => r.guardian_id as string | null),
    ...
    const petIds = [
      ...new Set(
        [
          ...(boardingRows ?? []).map((r) => r.pet_id as string | null),
          ...(apptRows ?? []).map((r) => r.pet_id as string | null),
        ].filter(Boolean) as string[]
      ),
    ];
    const guardianMap = new Map<string, { full_name: string; phone: string | null }>();
    const petMap = new Map<string, string>();
    if (guIds.length > 0) {
      const { data: gus } = await supabaseAdmin
        .from('hub_guardians')
        .select('id, full_name, phone')
        .in('id', guIds);
      for (const g of gus ?? []) {
        guardianMap.set(g.id as string, {
          full_name: String(g.full_name ?? ''),
          phone: (g.phone as string | null) ?? null,
        });
      }
    ...
    if (petIds.length > 0) {
      ...
      for (const p of pets ?? []) petMap.set(p.id as string, String(p.name ?? ''));
    }

    // Denominador para taxa: atendimentos "fechados" no período + no-shows
    let doneQ = supabaseAdmin
      .from('hub_appointments')
      ...
      .gte('starts_at', fromIso)
      .lte('starts_at', toIso);
    if (unit_id) doneQ = doneQ.or(`unit_id.eq.${unit_id},unit_id.is.null`);
    const { count: attendedCount } = await doneQ;

    let boardDoneQ = supabaseAdmin
      .from('hub_boarding_reservations')
      .select('id', { count: 'exact', head: true })
      .eq('clinic_id', clinic_id)
      ...
      .gte('expected_check_in', fromIso)
      .lte('expected_check_in', toIso);
    if (unit_id) boardDoneQ = boardDoneQ.eq('unit_id', unit_id);
    ...
    for (const r of boardingRows ?? []) {
      const g = r.guardian_id ? guardianMap.get(r.guardian_id as string) : null;
      // Prefer embedded join when present
      const embG = r.hub_guardians as { full_name?: string; phone?: string | null } | { full_name?: string; phone?: string | null }[] | null;
      const embGObj = Array.isArray(embG) ? embG[0] ?? null : embG;
      const embP = r.hub_pets as { name?: string } | { name?: string }[] | null;
      const embPObj = Array.isArray(embP) ? embP[0] ?? null : embP;
      items.push({
        id: r.id as string,
        source: 'boarding',
        kind_label: r.mode === 'daycare' ? 'Creche' : 'Hotel',
        when_at: r.expected_check_in as string,
        guardian_name: embGObj?.full_name ?? g?.full_name ?? null,
        phone: embGObj?.phone ?? g?.phone ?? null,
        pet_name: embPObj?.name ?? (r.pet_id ? petMap.get(r.pet_id as string) ?? null : null),
        status: 'no_show',
      });
    }
    for (const r of apptRows ?? []) {
      ...
      items.push({
        id: r.id as string,
        source: 'appointment',
        kind_label: (r.title as string | null)?.trim() || 'Agendamento',
        when_at: r.starts_at as string,
        guardian_name: g?.full_name ?? null,
        phone: g?.phone ?? null,
        pet_name: r.pet_id ? petMap.get(r.pet_id as string) ?? null : null,
...
export const getHubBoardingOccupancySeriesReport = async (req: Request, res: Response) => {
  try {
    const clinic = uuidStr.safeParse(req.query.clinic_id);
    if (!clinic.success) return res.status(400).json({ error: 'clinic_id inválido' });
    const clinic_id = clinic.data;
    ...
    for (let d = fromYmd; ; d = addDaysYmd(d, 1)) {
      ...
      for (const r of reservations ?? []) {
        const cin = Date.parse(r.expected_check_in as string);
        const cout = Date.parse(r.expected_check_out as string);
        if (Number.isNaN(cin) || Number.isNaN(cout)) continue;
        if (cin < dayEnd && cout > dayStart) {
          if (r.mode === 'hotel') hotel += 1;
          else if (r.mode === 'daycare') daycare += 1;
        }
      }
      ...
      if (hotel > peakHotel) peakHotel = hotel;
      if (daycare > peakDaycare) peakDaycare = daycare;
      if (hotel_pct != null) {
 ...
 */
export const getHubClientCohortsReport = async (req: Request, res: Response) => {
  try {
    const clinic = uuidStr.safeParse(req.query.clinic_id);
    if (!clinic.success) return res.status(400).json({ error: 'clinic_id inválido' });
    const clinic_id = clinic.data;
    ...
    const { data: newGuardians, error: ngErr } = await supabaseAdmin
      .from('hub_guardians')
      ...
      .order('created_at', { ascending: false })
      .limit(500);
    if (ngErr) return res.status(500).json({ error: ngErr.message });
    ...
    const firstSaleAt = new Map<string, string>();
    const priorGuardians = new Set<string>();
    const periodTotals = new Map<string, { amount: number; count: number }>();

    for (const row of recs ?? []) {
      const gid = row.guardian_id as string;
      const at = row.created_at as string;
      if (!firstSaleAt.has(gid)) firstSaleAt.set(gid, at);
      if (at < fromIso) priorGuardians.add(gid);
    ...
    const firstPurchase: typeof recurring = [];
    ...
    for (const gid of periodGuardianIds) {
      ...
      if (priorGuardians.has(gid)) recurring.push(row);
      else if (firstAt && firstAt >= fromIso && firstAt <= toIso) firstPurchase.push(row);
    }
    ...
    return res.json({
      ...
      new_clients: (newGuardians ?? []).map((g) => ({
        ...
        client_status: (g.client_status as string | null) ?? null,
      })),
...
function nextBirthdayOnOrAfter(birthYmd: string, fromYmd: string): string | null {
  const bm = birthYmd.slice(5, 7);
  const bd = birthYmd.slice(8, 10);
  if (!/^\d{2}$/.test(bm) || !/^\d{2}$/.test(bd)) return null;
  const fromY = Number(fromYmd.slice(0, 4));
  const tryYmd = (y: number) => {
    let day = Number(bd);
  ...
  if (cand < fromYmd) cand = tryYmd(fromY + 1);
  return cand;
}
 ...
 */
export const getHubBirthdaysReport = async (req: Request, res: Response) => {
  try {
    const clinic = uuidStr.safeParse(req.query.clinic_id);
    if (!clinic.success) return res.status(400).json({ error: 'clinic_id inválido' });
    const clinic_id = clinic.data;
    const daysParsed = z.coerce.number().int().min(1).max(90).safeParse(req.query.days);
    ...
    const petGuardian = new Map<string, { guardian_id: string; guardian_name: string }>();
    if (petIds.length > 0) {
      const chunk = 500;
      for (let i = 0; i < petIds.length; i += chunk) {
        ...
        for (const link of links ?? []) {
          const petId = link.pet_id as string;
          if (petGuardian.has(petId) && link.role !== 'primary') continue;
          const g = link.hub_guardians as { id?: string; full_name?: string } | { id?: string; full_name?: string }[] | null;
    ...
    for (const p of pets ?? []) {
      ...
      const next = nextBirthdayOnOrAfter(bd, asOf);
      if (!next || next > until) continue;
      const link = petGuardian.get(p.id as string);
      items.push({
        kind: 'pet',
        id: p.id as string,
        name: String(p.name ?? ''),
        ...
        species: (p.species as string | null) ?? null,
      });
    }
 ...
 */
export const getHubGroomingProductivityReport = async (req: Request, res: Response) => {
  try {
    const clinic = uuidStr.safeParse(req.query.clinic_id);
    if (!clinic.success) return res.status(400).json({ error: 'clinic_id inválido' });
    const clinic_id = clinic.data;
    ...
    let q = supabaseAdmin
      ...
      .select(
        'id, hub_staff_member_id, grooming_stage, started_at, closed_at, checked_in_at, unit_id, pet_id, guardian_id'
      )
      .eq('clinic_id', clinic_id)
      .is('deleted_at', null)
    ...
    const staffIds = [
      ...new Set(
        (sessions ?? [])
          .map((s) => s.hub_staff_member_id as string | null)
    ...
    if (staffIds.length > 0) {
      ...
      for (const s of staffRows ?? []) {
        staffNames.set(s.id as string, String(s.full_name ?? ''));
      }
    }
    ...
    const byDayStaff = new Map<string, { date: string; staff_id: string | null; staff_name: string; closed_count: number }>();
    ...
    for (const s of sessions ?? []) {
      closedTotal += 1;
      const sid = (s.hub_staff_member_id as string | null) ?? null;
      ...
      const closedAt = s.closed_at ? Date.parse(s.closed_at as string) : NaN;
      const startedAt = s.started_at
        ? Date.parse(s.started_at as string)
        : s.checked_in_at
          ? Date.parse(s.checked_in_at as string)
          : NaN;
    ...
    const by_staff = [...byStaff.values()]
      .map((a) => ({
        ...
        avg_duration_min:
          a.duration_samples > 0 ? Math.round(a.total_duration_min / a.duration_samples) : null,
      }))
 ...
 */
export const getHubVaccinesDueReport = async (req: Request, res: Response) => {
  try {
    const clinic = uuidStr.safeParse(req.query.clinic_id);
    if (!clinic.success) return res.status(400).json({ error: 'clinic_id inválido' });
    const clinic_id = clinic.data;
    const daysParsed = z.coerce.number().int().min(1).max(180).safeParse(req.query.days);
    const days = daysParsed.success ? daysParsed.data : 30;
    const asOf = ymdTodayUtc();
    const until = addDaysYmd(asOf, days);
    ...
    const { data: rows, error } = await supabaseAdmin
      ...
      .select(
        'id, pet_id, vaccine_name, administered_at, next_dose_at, batch_number, source, hub_pets(id, name, species)'
      )
      .eq('clinic_id', clinic_id)
      .is('deleted_at', null)
    ...
    const petGuardian = new Map<string, { guardian_id: string; guardian_name: string; phone: string | null }>();
    if (petIds.length > 0) {
      const { data: links, error: lErr } = await supabaseAdmin
        .from('hub_pet_guardians')
    ...
    const items = (rows ?? []).map((r) => {
      ...
      const link = petGuardian.get(r.pet_id as string);
      return {
        id: r.id as string,
        ...
        administered_at: r.administered_at as string,
        next_dose_at: next,
        days_until: daysBetweenYmd(asOf, next),
        overdue,
