...
exports.getAdminUsage = exports.getAdminSpecialties = exports.getAdminOverview = void 0;
...
const getAdminOverview = async (req, res) => {
    ...
    try {
        ...
        const { data: allClinics, error: clinicsError } = await supabase_1.supabase
            .from('clinics')
        ...
        const { data: clinicsWithDemands } = await supabase_1.supabase
            .from('demands')
            .select('clinic_id')
        ...
        const { data: clinicUsersWithActivity } = await supabase_1.supabase
            .from('clinic_users')
            .select('clinic_id')
        ...
        clinicUsersWithActivity?.forEach(cu => {
            if (cu.clinic_id)
                activeClinicIds.add(cu.clinic_id);
        });
        // Count active clinics (status active AND has activity)
        const activeClinics = allClinics?.filter(c => c.status === 'active' && activeClinicIds.has(c.id)).length || 0;
        // Count inactive clinics (status active but no activity, or status inactive)
        const inactiveClinics = allClinics?.filter(c => c.status === 'active' && !activeClinicIds.has(c.id)).length || 0;
        // Get new registrations in period
        const { data: newClinics, error: newClinicsError } = await supabase_1.supabase
            .from('clinics')
            .select('id')
        ...
        const { data: newUnits, error: newUnitsError } = await supabase_1.supabase
            .from('units')
            .select('id')
            .gte('created_at', start.toISOString())
            .lte('created_at', end.toISOString());
        if (newUnitsError)
            throw newUnitsError;
        const { data: newVets, error: newVetsError } = await supabase_1.supabase
            .from('vets')
            .select('id')
            .gte('created_at', start.toISOString())
            .lte('created_at', end.toISOString());
        ...
        const { data: previousNewClinics } = await supabase_1.supabase
            .from('clinics')
            .select('id')
            .gte('created_at', previousPeriodStart.toISOString())
            .lte('created_at', previousPeriodEnd.toISOString())
            .is('deleted_at', null);
        const { data: previousNewUnits } = await supabase_1.supabase
            .from('units')
            .select('id')
            .gte('created_at', previousPeriodStart.toISOString())
            .lte('created_at', previousPeriodEnd.toISOString());
        const { data: previousNewVets } = await supabase_1.supabase
            .from('vets')
            .select('id')
            .gte('created_at', previousPeriodStart.toISOString())
            .lte('created_at', previousPeriodEnd.toISOString());
        ...
        if (demandIds.length > 0) {
            const { data: positionsData, error: positionsError } = await supabase_1.supabase
...
exports.getAdminSpecialties = getAdminSpecialties;
...
const getAdminUsage = async (req, res) => {
    ...
    try {
        ...
        const { data: activeClinicUsers, error: clinicUsersError } = await supabase_1.supabase
            .from('clinic_users')
            .select('user_id')
            .or(`first_login_at.gte.${thirtyDaysAgo.toISOString()},updated_at.gte.${thirtyDaysAgo.toISOString()}`);
        if (clinicUsersError)
            throw clinicUsersError;
        ...
        const { data: activeVets, error: vetsError } = await supabase_1.supabase
            .from('vets')
            .select('id')
        ...
        activeClinicUsers?.forEach(cu => {
            if (cu.user_id)
                activeUserIds.add(cu.user_id);
        });
        activeVets?.forEach(v => {
            if (v.id)
                activeUserIds.add(v.id);
        });
        ...
        const { data: loginsInPeriod, error: loginsError } = await supabase_1.supabase
            .from('clinic_users')
            .select('user_id')
