...
exports.getConversationMetadata = exports.reportMessage = exports.deleteMessage = exports.archiveConversation = exports.markAsRead = exports.sendMessage = exports.getConversation = exports.getMyConversations = exports.createConversation = exports.validateConversationPair = void 0;
const supabase_1 = require("../config/supabase");
// ========================================
// VALIDAÇÃO DE PARES PERMITIDOS
// ========================================
const validateConversationPair = (type1, type2) => {
    ...
    if ((type1 === 'clinic' && (type2 === 'vet' || type2 === 'freelancer')) ||
...
exports.validateConversationPair = validateConversationPair;
// ========================================
// CRIAR CONVERSA
// ========================================
const createConversation = async (req, res) => {
    try {
        ...
        const validation = (0, exports.validateConversationPair)(p1_type_normalized, p2_type_normalized);
        if (!validation.valid) {
        ...
        const { data: existing } = await supabase_1.supabase
            .from('conversations')
            .select('id')
            .or(`and(participant1_id.eq.${p1_id},participant2_id.eq.${p2_id}),and(participant1_id.eq.${p2_id},participant2_id.eq.${p1_id})`)
            .maybeSingle();
        if (existing) {
        ...
        const isValidOrder = (final_p1_type === 'admin' && ['clinic', 'vet', 'freelancer'].includes(final_p2_type)) ||
...
const getMyConversations = async (req, res) => {
    try {
        ...
        const enrichedConversations = await Promise.all((conversations || []).map(async (conv) => {
            const otherParticipantId = conv.participant1_id === user_id ? conv.participant2_id : conv.participant1_id;
            const otherParticipantType = conv.participant1_id === user_id ? conv.participant2_type : conv.participant1_type;
            // Buscar informações do outro participante
            let otherParticipant = null;
            if (otherParticipantType === 'clinic') {
                const { data } = await supabase_1.supabase.from('clinics').select('id, name, photo_url').eq('id', otherParticipantId).single();
                otherParticipant = data;
            }
            else if (otherParticipantType === 'vet') {
                const { data } = await supabase_1.supabase.from('vets').select('id, name, photo_url').eq('id', otherParticipantId).single();
                otherParticipant = data;
            }
            else if (otherParticipantType === 'freelancer') {
                const { data } = await supabase_1.supabase.from('freelancers').select('id, name, photo_url').eq('id', otherParticipantId).single();
                otherParticipant = data;
            }
            else if (otherParticipantType === 'admin') {
                // Admin não tem tabela própria, buscar de auth.users
                const { data: userData } = await supabase_1.supabaseAdmin.auth.admin.getUserById(otherParticipantId);
                otherParticipant = {
            ...
            let demand = null;
            if (conv.demand_id) {
                const { data: demandData } = await supabase_1.supabase
                    .from('demands')
                    .select('id, title')
    ...
    try {
        ...
        const enrichedMessages = await Promise.all((messages || []).map(async (msg) => {
            ...
            if (msg.sender_type === 'clinic') {
                const { data } = await supabase_1.supabase.from('clinics').select('id, name, photo_url').eq('id', msg.sender_id).single();
                sender = data;
            }
            else if (msg.sender_type === 'vet') {
                const { data } = await supabase_1.supabase.from('vets').select('id, name, photo_url').eq('id', msg.sender_id).single();
                sender = data;
            }
            else if (msg.sender_type === 'freelancer') {
                const { data } = await supabase_1.supabase.from('freelancers').select('id, name, photo_url').eq('id', msg.sender_id).single();
                sender = data;
            }
            else if (msg.sender_type === 'admin') {
                // Admin não tem tabela própria, buscar de auth.users
                const { data: userData } = await supabase_1.supabaseAdmin.auth.admin.getUserById(msg.sender_id);
                sender = {
                    id: msg.sender_id,
                    name: userData?.user?.user_metadata?.name || userData?.user?.email?.split('@')[0] || 'Administrador',
                    photo_url: userData?.user?.user_metadata?.photo_url || null,
...
const sendMessage = async (req, res) => {
    try {
        ...
        const sender_type = conversation.participant1_id === user_id
            ? conversation.participant1_type
            : conversation.participant2_type;
        // Criar mensagem
        const { data: newMessage, error: msgError } = await supabase_1.supabase
            .from('messages')
            .insert([
            {
...
const markAsRead = async (req, res) => {
    try {
        ...
        const otherParticipantId = conversation.participant1_id === user_id
...
const getConversationMetadata = async (req, res) => {
    try {
        ...
        const { count: messageCount } = await supabase_1.supabase
            .from('messages')
            .select('*', { count: 'exact', head: true })
            .eq('conversation_id', conversationId)
            .is('deleted_at', null);
