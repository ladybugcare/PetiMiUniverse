...
exports.getPendingUnits = void 0;
...
const getPendingUnits = async (req, res) => {
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Query timeout')), 25000));
        // 1) Unidades explicitamente em revisão
        const qReview = supabase_1.supabaseAdmin
            .from('units')
        ...
        const qActiveMain = supabase_1.supabaseAdmin
            .from('units')
            .select(PENDING_UNITS_SELECT)
            .eq('status', 'active')
            .eq('is_main', true)
        ...
        for (const u of r2.data || []) {
            const st = u?.clinic?.status;
            if (st === 'pending_approval') {
                byId.set(u.id, u);
            }
        }
        const merged = Array.from(byId.values()).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
