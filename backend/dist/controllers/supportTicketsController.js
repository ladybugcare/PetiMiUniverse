...
const createTicket = async (req, res) => {
    try {
        ...
        const { error: messageError } = await supabase_1.supabase
    ...
    try {
        ...
        const ticketsWithDetails = await Promise.all((tickets || []).map(async (ticket) => {
            ...
            const { data: lastMessage } = await supabase_1.supabase
                ...
                .single();
            // Buscar avaliação
            const { data: evaluation } = await supabase_1.supabase
                .from('ticket_evaluations')
                .select('*')
                .eq('ticket_id', ticket.id)
                .single();
            // Contar mensagens não lidas do admin
            const { data: unreadMessages } = await supabase_1.supabase
                .from('ticket_messages')
                .select('id')
                .eq('ticket_id', ticket.id)
...
const getTicketsCount = async (req, res) => {
    try {
        ...
        const { data: tickets, error } = await supabase_1.supabase
            .from('support_tickets')
            .select('status');
        if (error) {
...
exports.markTicketAsRead = markTicketAsRead;
// ========================================
...
const getUnreadCount = async (req, res) => {
    try {
        ...
        const { data: unreadMessages, error: messagesError } = await supabase_1.supabase
            .from('ticket_messages')
            .select('id')
...
const addMessage = async (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        // Criar mensagem
        const { data: newMessage, error: messageError } = await supabase_1.supabase
            .from('ticket_messages')
            .insert([
            {
                ticket_id,
...
const createEvaluation = async (req, res) => {
    try {
        ...
        const { data: existingEvaluation } = await supabase_1.supabase
            .from('ticket_evaluations')
            .select('id')
            .eq('ticket_id', ticket_id)
            .single();
        ...
        const now = new Date().toISOString();
        // Criar avaliação
        const { data: evaluation, error: evaluationError } = await supabase_1.supabase
            .from('ticket_evaluations')
            .insert([
            {
                ticket_id,
        ...
        const { data: updatedTicket, error: updateError } = await supabase_1.supabase
            .from('support_tickets')
            .update({
            status: 'resolved',
            resolved_at: now,
