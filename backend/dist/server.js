...
function portForAttempt(attemptIndex) {
    if (explicitPort) {
...
function start(attemptIndex) {
    const port = portForAttempt(attemptIndex);
    const server = http_1.default.createServer(app_1.default);
    server.listen(port, () => {
        console.log(`🐾 Server running on port ${port}`);
        if (!explicitPort && !isProd && port !== 3000) {
            console.warn(`⚠️  Porta 3000 ocupada — API em http://localhost:${port}. Defina REACT_APP_API_URL (frontend) para o mesmo host/porta.`);
        }
    });
    server.on('error', (err) => {
