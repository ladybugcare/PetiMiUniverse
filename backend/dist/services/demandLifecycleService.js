...
class DemandLifecycleService {
    ...
    static async updateDemandStatus(demandId, newStatus) {
        try {
            ...
            if (error)
                throw error;
            logger_js_1.logger.info('Status da demanda atualizado', {
                demandId,
                newStatus,
            });
        }
        catch (error) {
            logger_js_1.logger.error('Erro ao atualizar status da demanda', {
                demandId,
                newStatus,
                error: error.message,
            });
    ...
    static validateStatusTransition(currentStatus, newStatus) {
        ...
        if (!allowedStatuses.includes(newStatus)) {
            throw new Error(`Transição inválida: não é possível mudar de "${currentStatus}" para "${newStatus}". ` +
    ...
    static async updateApplicationStatus(applicationId, newStatus) {
        try {
            // Buscar status atual
            const { data: application, error: fetchError } = await supabase_js_1.supabaseAdmin
     ...
     */
    static async calculateDemandStatus(demandId) {
        try {
            // Buscar demanda
            ...
            // Buscar aplicações
            const { data: applications, error: appsError } = await supabase_js_1.supabaseAdmin
                .from('demand_applications')
                .select('status')
            ...
            logger_js_1.logger.error('Erro ao calcular status da demanda', {
                demandId,
                error: error.message,
            });
            throw error;
     ...
     * @param demandId ID da demanda
     */
    static async transitionToInProgress(demandId) {
        try {
            ...
            logger_js_1.logger.error('Erro ao transicionar para in_progress', {
                demandId,
                error: error.message,
            });
     ...
     * @param demandId ID da demanda
     */
    static async transitionToAwaitingReport(demandId) {
        try {
            // Buscar demanda
            const { data: demand } = await supabase_js_1.supabaseAdmin
                .from('demands')
            ...
            const { data: approvedApps } = await supabase_js_1.supabaseAdmin
                .from('demand_applications')
                .select('status')
                .eq('demand_id', demandId)
            ...
            const allCheckedOut = approvedApps.every((app) => ['check_out', 'report_sent', 'report_approved'].includes(app.status));
            if (allCheckedOut) {
                await this.updateDemandStatus(demandId, 'awaiting_report');
            }
        }
        catch (error) {
            logger_js_1.logger.error('Erro ao transicionar para awaiting_report', {
                demandId,
                error: error.message,
            });
            // Não falhar a operação principal
     ...
     */
    static async checkTimeConflict(applicationId) {
        try {
        ...
        `)
            ...
            const { data: conflicts, error: conflictError } = await supabase_js_1.supabaseAdmin
            ...
            logger_js_1.logger.error('Erro ao verificar conflitos de horário', {
                applicationId,
                error: error.message,
            });
            throw error;
     ...
     * @param demandId ID da demanda
     */
    static async transitionToCompleted(demandId) {
        try {
            // Buscar aplicações aprovadas
            const { data: approvedApps } = await supabase_js_1.supabaseAdmin
                .from('demand_applications')
            ...
            logger_js_1.logger.error('Erro ao transicionar para completed', {
                demandId,
                error: error.message,
            });
            // Não falhar a operação principal
...
exports.DemandLifecycleService = DemandLifecycleService;
