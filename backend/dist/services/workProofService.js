...
exports.WorkProofService = void 0;
const supabase_js_1 = require("../config/supabase.js");
const notificationsController_js_1 = require("../controllers/notificationsController.js");
const logger_js_1 = require("../utils/logger.js");
const demandLifecycleService_js_1 = require("./demandLifecycleService.js");
/**
...
class WorkProofService {
     ...
     */
    static async checkIn(applicationId, location) {
        try {
            ...
            const { data: existingProof } = await supabase_js_1.supabaseAdmin
                .from('work_proof')
            ...
            if (existingProof) {
                ...
                const { data: created, error: createError } = await supabase_js_1.supabaseAdmin
                    .from('work_proof')
                    .insert({
            ...
            const { error: statusError } = await supabase_js_1.supabaseAdmin
                .from('demand_applications')
                .update({
                status: 'check_in',
                updated_at: new Date().toISOString(),
            ...
            await demandLifecycleService_js_1.DemandLifecycleService.transitionToInProgress(demand.id);
            // Recalcular status da demanda
            try {
                const newStatus = await demandLifecycleService_js_1.DemandLifecycleService.calculateDemandStatus(demand.id);
                await demandLifecycleService_js_1.DemandLifecycleService.updateDemandStatus(demand.id, newStatus);
            }
            catch (statusError) {
                logger_js_1.logger.error('Erro ao recalcular status da demanda após check-in', {
                    demandId: demand.id,
            ...
            await (0, notificationsController_js_1.createNotification)({
                ...
                message: `O veterinário fez check-in na demanda "${demand.title}"`,
                link: `/demands/${demand.id}`,
                entity_type: 'application',
                entity_id: applicationId,
            });
            logger_js_1.logger.info('Check-in registrado', {
                applicationId,
                demandId: demand.id,
            ...
            logger_js_1.logger.error('Erro ao registrar check-in', {
                applicationId,
                error: error.message,
            });
            throw error;
     ...
     * @returns Work proof atualizado
     */
    static async checkOut(applicationId, location) {
        try {
            // Verificar se aplicação existe e está no status correto
            const { data: application, error: appError } = await supabase_js_1.supabaseAdmin
                .from('demand_applications')
            ...
            // Buscar work_proof
            const { data: workProof, error: proofError } = await supabase_js_1.supabaseAdmin
                .from('work_proof')
                .select('id')
                .eq('application_id', applicationId)
                .single();
            ...
            const demand = application.demands;
            await demandLifecycleService_js_1.DemandLifecycleService.transitionToAwaitingReport(demand.id);
            // Recalcular status da demanda
            try {
                const newStatus = await demandLifecycleService_js_1.DemandLifecycleService.calculateDemandStatus(demand.id);
                await demandLifecycleService_js_1.DemandLifecycleService.updateDemandStatus(demand.id, newStatus);
                ...
                logger_js_1.logger.error('Erro ao recalcular status da demanda após check-out', {
                    demandId: demand.id,
                    error: statusError.message,
                });
            }
            logger_js_1.logger.info('Check-out registrado', {
                applicationId,
                demandId: demand.id,
                location: location ? 'sim' : 'não',
            });
            ...
            logger_js_1.logger.error('Erro ao registrar check-out', {
                applicationId,
                error: error.message,
            });
            throw error;
     ...
     * @returns Work proof atualizado
     */
    static async submitReport(applicationId, reportText, attachments) {
        try {
            // Verificar se aplicação existe e está no status correto
            const { data: application, error: appError } = await supabase_js_1.supabaseAdmin
                .from('demand_applications')
            ...
            if (application.status !== 'check_out') {
                demandLifecycleService_js_1.DemandLifecycleService.validateStatusTransition(application.status, 'report_sent');
            }
            ...
            try {
                ...
                logger_js_1.logger.error('Erro ao recalcular status da demanda após envio de relatório', {
                    demandId: demand.id,
                    error: statusError.message,
                });
            }
            ...
            await (0, notificationsController_js_1.createNotification)({
                ...
                message: `O veterinário enviou o relatório da demanda "${demand.title}"`,
                link: `/demands/${demand.id}`,
                entity_type: 'application',
                entity_id: applicationId,
            });
            logger_js_1.logger.info('Relatório enviado', {
                applicationId,
                demandId: demand.id,
            ...
            logger_js_1.logger.error('Erro ao enviar relatório', {
                applicationId,
                error: error.message,
            });
            throw error;
     ...
     */
    static async approveReport(applicationId, approvedBy) {
        try {
            // Verificar se aplicação existe e está no status correto
            const { data: application, error: appError } = await supabase_js_1.supabaseAdmin
                .from('demand_applications')
            ...
            if (application.status !== 'report_sent') {
                demandLifecycleService_js_1.DemandLifecycleService.validateStatusTransition(application.status, 'report_approved');
            }
            ...
            const { data: workProof } = await supabase_js_1.supabaseAdmin
                .from('work_proof')
                .select('id')
                .eq('application_id', applicationId)
                .single();
            ...
            // Atualizar status da aplicação
            const { data: updated, error: statusError } = await supabase_js_1.supabaseAdmin
                .from('demand_applications')
                .update({
            ...
            await demandLifecycleService_js_1.DemandLifecycleService.transitionToCompleted(demand.id);
            // Recalcular status da demanda
            try {
                const newStatus = await demandLifecycleService_js_1.DemandLifecycleService.calculateDemandStatus(demand.id);
                await demandLifecycleService_js_1.DemandLifecycleService.updateDemandStatus(demand.id, newStatus);
                ...
                logger_js_1.logger.error('Erro ao recalcular status da demanda após aprovação de relatório', {
                    demandId: demand.id,
                    error: statusError.message,
                });
            }
            ...
            await (0, notificationsController_js_1.createNotification)({
                ...
                message: `Seu relatório da demanda "${demand.title}" foi aprovado`,
                link: `/my-applications`,
                entity_type: 'application',
                entity_id: applicationId,
            });
            logger_js_1.logger.info('Relatório aprovado', {
                applicationId,
                demandId: demand.id,
                approvedBy,
            });
            ...
            logger_js_1.logger.error('Erro ao aprovar relatório', {
                applicationId,
                approvedBy,
                error: error.message,
            });
     ...
     */
    static async getWorkProof(applicationId) {
        try {
            const { data: workProof, error } = await supabase_js_1.supabaseAdmin
                .from('work_proof')
                .select('*')
                .eq('application_id', applicationId)
                .single();
            ...
            logger_js_1.logger.error('Erro ao buscar work proof', {
                applicationId,
                error: error.message,
            });
            throw error;
...
exports.WorkProofService = WorkProofService;
