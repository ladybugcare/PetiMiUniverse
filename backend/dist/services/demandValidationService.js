...
exports.DemandValidationService = void 0;
...
class DemandValidationService {
     ...
     */
    static async validateClinic(clinicId, userId) {
        ...
        if (!clinicOperational && clinic.status === 'pending_approval') {
            const { data: approvedUnits, error: unitsErr } = await supabase_js_1.supabaseAdmin
                .from('units')
                .select('id')
     ...
     */
    static async validateUnit(unitId, clinicId) {
        // Buscar unidades aprovadas da clínica
        const { data: units, error: unitsError } = await supabase_js_1.supabaseAdmin
            .from('units')
            .select('id')
            .eq('clinic_id', clinicId)
            .in('status', ['approved', 'active']);
     ...
     */
    static validateDate(demandDate) {
     ...
     */
    static async validateSpecialties(specialties, category) {
        if (!specialties || specialties.length === 0) {
            throw new Error('É necessário especificar pelo menos uma especialidade');
        }
        // Buscar todas as especialidades fornecidas
        const { data: foundSpecialties, error } = await supabase_js_1.supabaseAdmin
            .from('specialties')
        ...
        if (notFound.length > 0) {
            throw new Error(`Especialidades não encontradas: ${notFound.join(', ')}`);
        }
        // Verificar se todas estão ativas
        const inactive = foundSpecialties?.filter((s) => !s.active) || [];
        if (inactive.length > 0) {
            throw new Error(`Especialidades inativas: ${inactive.map((s) => s.name).join(', ')}`);
        }
        ...
        if (wrongCategory.length > 0) {
            throw new Error(`Especialidades não pertencem à categoria "${category}": ${wrongCategory
                .map((s) => s.name)
                .join(', ')}`);
        }
    }
    ...
    static validatePositions(positions) {
        ...
        positions.forEach((pos, index) => {
            if (!pos.slots || pos.slots < 1) {
                throw new Error(`Posição ${index + 1}: número de vagas deve ser maior que zero`);
            }
            if (!pos.specialties || pos.specialties.length === 0) {
                throw new Error(`Posição ${index + 1}: é necessário especificar pelo menos uma especialidade`);
            }
        });
    }
    /**
     ...
     */
    static calculateVacancies(positions) {
        return positions.reduce((total, pos) => total + (pos.slots || 0), 0);
    }
    /**
     * Valida o intervalo de horários
     */
    static validateTimeRange(startTime, endTime, isOvernight) {
...
exports.DemandValidationService = DemandValidationService;
