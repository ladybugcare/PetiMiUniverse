...
exports.getHubSessionContext = (0, errorHandler_js_1.asyncHandler)(async (req, res) => {
    ...
    const list = rows || [];
    let clinicUser = pickClinicUserRow(list);
    let resolvedClinicId = clinicUser?.clinic_id ||
    ...
    if (!resolvedClinicId && isClinicOwnerMetadata && userEmail) {
        const { data: clinicByEmail } = await supabase_js_1.supabaseAdmin
            .from('clinics')
            .select('id')
    ...
    if (!resolvedClinicId && isClinicOwnerMetadata) {
        const { data: clinicByOwnerId } = await supabase_js_1.supabaseAdmin
            .from('clinics')
            .select('id')
            .eq('id', userId)
            .maybeSingle();
    ...
    if (resolvedClinicId) {
        const [{ data: clinic }, { count: unitCount }] = await Promise.all([
            supabase_js_1.supabaseAdmin.from('clinics').select('status').eq('id', resolvedClinicId).maybeSingle(),
    ...
    const clinicUserPayload = clinicUser
        ? {
            id: clinicUser.id,
            clinic_id: clinicUser.clinic_id ?? resolvedClinicId,
