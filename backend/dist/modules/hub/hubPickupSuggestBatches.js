...
exports.windowBucketMs = windowBucketMs;
exports.timeBucketKey = timeBucketKey;
exports.partitionStopsIntoBatches = partitionStopsIntoBatches;
const NO_TIME_SENTINEL = Number.POSITIVE_INFINITY;
function parseStartMs(startsAt) {
    if (!startsAt)
...
function windowBucketMs(windowMinutes) {
    if (windowMinutes === 0)
        return 60_000;
    return windowMinutes * 60_000;
}
function timeBucketKey(startsAtMs, windowMinutes) {
...
function formatBucketLabel(bucketStartMs) {
 ...
 */
function partitionStopsIntoBatches(stops, opts) {
    ...
    for (const stop of ordered) {
        ...
        const needsNewForBucket = current != null && current.stop_ids.length > 0 && current.bucketKey !== bucketKey;
        const needsNewForCapacity = current != null && current.pet_count >= capacity && current.stop_ids.length > 0;
