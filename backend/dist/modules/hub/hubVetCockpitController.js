...
exports.getHubVetCockpitPatientContext = getHubVetCockpitPatientContext;
const zod_1 = require("zod");
const supabase_js_1 = require("../../config/supabase.js");
const uuidStr = zod_1.z.string().uuid();
const patientContextQuerySchema = zod_1.z.object({
    clinic_id: uuidStr,
    pet_id: uuidStr.optional(),
    encounter_id: uuidStr.optional(),
    appointment_id: uuidStr.optional(),
});
const PET_SELECT = 'id, name, species, breed, sex, birth_date, size_tier, coat_type, notes';
/** GET /clinical/cockpit/patient-context */
async function getHubVetCockpitPatientContext(req, res) {
    try {
        ...
        if (encounter_id) {
            const { data: enc, error: encErr } = await supabase_js_1.supabaseAdmin
                .from('hub_encounters')
                .select('id, pet_id, guardian_id, chief_complaint, summary_notes, hub_case_id, status, operational_phase, physical_exam, hub_appointment_id')
                .eq('id', encounter_id)
            ...
            encounterRow = enc;
            resolvedPetId = enc.pet_id ?? resolvedPetId;
            resolvedGuardianId = enc.guardian_id ?? null;
            chiefComplaint = enc.chief_complaint ?? null;
        }
        if (appointment_id) {
            const { data: appt, error: apptErr } = await supabase_js_1.supabaseAdmin
            ...
            appointmentRow = appt;
            if (appt) {
                resolvedPetId = resolvedPetId ?? appt.pet_id;
                resolvedGuardianId = resolvedGuardianId ?? appt.guardian_id;
                if (!chiefComplaint) {
                    chiefComplaint = appt.notes ?? appt.title;
                }
            }
        }
        if (!resolvedPetId) {
        ...
        const [petRes, guardianRes, flagsRes, casesRes, encountersRes, examsRes, rxRes, vacRes, hospRes,] = await Promise.all([
        ...
        const physicalExam = (encounterRow?.physical_exam ?? {});
        ...
        return res.status(500).json({ error: e?.message || 'Erro ao carregar contexto do paciente' });
    }
}
