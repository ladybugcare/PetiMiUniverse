...
exports.getHubInventoryTurnoverReport = exports.getHubInventoryAbcReport = exports.listHubInventoryLots = exports.listHubLowStock = exports.listHubExpiringLots = exports.createHubStockMovement = exports.getHubInventoryMovementsReport = exports.listHubStockMovements = exports.patchHubInventoryItem = exports.createHubInventoryItem = exports.listHubInventoryItems = exports.patchHubManufacturer = exports.createHubManufacturer = exports.listHubManufacturers = exports.patchHubSupplier = exports.createHubSupplier = exports.listHubSuppliers = void 0;
exports.movementSign = movementSign;
exports.computeBalances = computeBalances;
const zod_1 = require("zod");
const supabase_1 = require("../../config/supabase");
const hubInventoryStockAlerts_1 = require("./hubInventoryStockAlerts");
...
const listHubStockMovements = async (req, res) => {
    try {
        ...
        const daysParsed = zod_1.z.coerce.number().int().min(1).max(366).safeParse(req.query.days);
        let fromIso = null;
        let toIso = null;
...
const getHubInventoryMovementsReport = async (req, res) => {
    try {
        ...
        const { data, error } = await supabase_1.supabaseAdmin
            .from('hub_stock_movements')
            .select('id, clinic_id, item_id, lot_id, movement_type, qty, unit_cost, notes, created_at, item:hub_inventory_items(id, name, item_kind, unit_label)')
            .eq('clinic_id', clinic_id)
...
exports.getHubInventoryMovementsReport = getHubInventoryMovementsReport;
const createHubStockMovement = async (req, res) => {
    try {
        ...
        if (sign < 0) {
            void (0, hubInventoryStockAlerts_1.notifyLowStockIfCrossed)({
...
function round2inv(n) {
    return Math.round(n * 100) / 100;
}
/** Curva ABC por valor de consumo (saídas × custo) no período. */
const getHubInventoryAbcReport = async (req, res) => {
    try {
        const clinic = uuidStr.safeParse(req.query.clinic_id);
        if (!clinic.success)
            return res.status(400).json({ error: 'clinic_id inválido' });
        ...
        const fromIso = `${fromYmd}T00:00:00.000Z`;
        const toIso = `${toYmd}T23:59:59.999Z`;
        const { data: movs, error } = await supabase_1.supabaseAdmin
            .from('hub_stock_movements')
...
exports.getHubInventoryAbcReport = getHubInventoryAbcReport;
/** Giro de estoque: saídas no período vs saldo atual / cobertura em dias. */
const getHubInventoryTurnoverReport = async (req, res) => {
    try {
        const clinic = uuidStr.safeParse(req.query.clinic_id);
        if (!clinic.success)
            return res.status(400).json({ error: 'clinic_id inválido' });
        ...
        const fromIso = `${fromYmd}T00:00:00.000Z`;
        const toIso = `${toYmd}T23:59:59.999Z`;
        const periodDays = Math.max(1, Math.floor((Date.parse(`${toYmd}T12:00:00Z`) - Date.parse(`${fromYmd}T12:00:00Z`)) / 86_400_000) + 1);
        const { data: movs, error } = await supabase_1.supabaseAdmin
            .from('hub_stock_movements')
        ...
        const rows = [...itemMeta.keys()]
            ...
            const turnover = avg_stock > 0 ? round2inv(qty_out / avg_stock) : qty_out > 0 ? null : 0;
            const daily_out = qty_out / periodDays;
            const days_of_cover = daily_out > 0 ? round2inv(Math.max(qty_on_hand, 0) / daily_out) : qty_on_hand > 0 ? null : 0;
            const meta = itemMeta.get(id);
            return {
                item_id: id,
                name: meta.name,
        ...
        return res.json({
            ...
            summary: {
                ...
                avg_turnover: rows.filter((r) => r.turnover_rate != null).length > 0
                    ? round2inv(rows
                        .filter((r) => r.turnover_rate != null)
                        .reduce((acc, r) => acc + r.turnover_rate, 0) /
...
exports.getHubInventoryTurnoverReport = getHubInventoryTurnoverReport;
