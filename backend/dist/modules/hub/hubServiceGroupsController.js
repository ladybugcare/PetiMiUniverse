...
exports.deleteHubServiceGroup = exports.patchHubServiceGroup = exports.createHubServiceGroup = exports.listHubServiceGroups = exports.DEFAULT_HUB_SERVICE_GROUP_ROWS = void 0;
...
const patchBodySchema = zod_1.z
    ...
    .strict();
const GROUP_SELECT = 'id, clinic_id, name, slug, color, display_order, archived_at, created_at, updated_at';
...
const listHubServiceGroups = async (req, res) => {
    try {
        ...
        const service_groups = (groups ?? []).map((row) => ({
            ...row,
            service_count: counts.get(row.slug) ?? 0,
        }));
...
const createHubServiceGroup = async (req, res) => {
    try {
        ...
        const { data, error } = await supabase_1.supabaseAdmin.from('hub_service_groups').insert([row]).select(GROUP_SELECT).single();
        if (error) {
            if (error.code === '23505') {
...
const patchHubServiceGroup = async (req, res) => {
    try {
        ...
        const service_count = await countServicesUsingGroupSlug(clinic_id, existing.slug);
