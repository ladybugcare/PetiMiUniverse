...
const optionalBirthDate = zod_1.z
    .union([zod_1.z.string(), zod_1.z.null(), zod_1.z.undefined()])
    .optional()
    .transform((v) => {
...
const createStaffSchema = zod_1.z
    .object({
    clinic_id: uuidStr,
    full_name: zod_1.z.string().trim().min(1, 'Nome completo é obrigatório').max(200),
    ...
    email: zod_1.z.union([zod_1.z.string().email().max(254), zod_1.z.literal(''), zod_1.z.null()]).optional().nullable(),
    ...
    break_minutes: zod_1.z.coerce.number().int().min(0).max(24 * 60).optional().nullable(),
    default_unit_id: uuidStr.optional().nullable(),
    agenda_color: optionalTrim(16).optional(),
    service_type_ids: zod_1.z.array(uuidStr).optional(),
})
    .strict();
...
async function fetchActiveServiceTypeIdsForClinic(clinicId, ids) {
...
async function replaceStaffServiceTypes(staffId, clinicId, serviceTypeIds) {
    ...
    const { error } = await supabase_1.supabaseAdmin.from('hub_staff_service_types').insert(rows);
    if (error)
        throw error;
}
async function enrichStaffRows(clinicId, rows) {
    if (rows.length === 0)
        return [];
    ...
    const { data: links } = await supabase_1.supabaseAdmin
        ...
        .in('staff_id', ids);
    const typeIds = [...new Set((links ?? []).map((l) => l.service_type_id))];
    let typeMap = new Map();
    if (typeIds.length > 0) {
        const { data: types } = await supabase_1.supabaseAdmin
            .from('hub_service_types')
            .select('id, name, code')
            .eq('clinic_id', clinicId)
            .in('id', typeIds);
        for (const t of types ?? []) {
            typeMap.set(t.id, { id: t.id, name: t.name, code: t.code });
        }
    }
    const byStaff = new Map();
    for (const l of links ?? []) {
        const sid = l.staff_id;
        const tid = l.service_type_id;
    ...
    const unitIds = [...new Set(rows.map((r) => r.default_unit_id).filter(Boolean))];
    let unitNameById = new Map();
    if (unitIds.length > 0) {
        const { data: units } = await supabase_1.supabaseAdmin.from('units').select('id, name').eq('clinic_id', clinicId).in('id', unitIds);
...
const listHubStaff = async (req, res) => {
    try {
...
exports.listHubStaff = listHubStaff;
const getHubStaff = async (req, res) => {
    try {
        ...
        const [enriched] = await enrichStaffRows(clinicParsed.data, [data]);
...
const createHubStaff = async (req, res) => {
    try {
        ...
        const hubEmailNorm = d.hub_access_email === '' || d.hub_access_email === undefined ? null : d.hub_access_email;
        if (d.default_unit_id) {
            const { data: u } = await supabase_1.supabaseAdmin.from('units').select('id, clinic_id').eq('id', d.default_unit_id).maybeSingle();
            if (!u || u.clinic_id !== d.clinic_id)
                return res.status(400).json({ error: 'Unidade inválida para esta clínica' });
        }
        ...
        const { data: created, error } = await supabase_1.supabaseAdmin.from('hub_staff_members').insert([insertRow]).select(STAFF_SELECT).single();
        if (error || !created) {
...
const patchHubStaff = async (req, res) => {
    try {
        const idParsed = uuidStr.safeParse(req.params.id);
        ...
        if (!clinic_id)
            return res.status(400).json({ error: 'clinic_id é obrigatório no corpo' });
        ...
        if (d.display_name !== undefined)
            patch.display_name = d.display_name;
        if (d.photo_url !== undefined)
            patch.photo_url = d.photo_url;
        if (d.phone !== undefined)
            patch.phone = d.phone;
        if (d.whatsapp_phone !== undefined)
            patch.whatsapp_phone = d.whatsapp_phone;
        if (d.email !== undefined)
            patch.email = d.email === '' ? null : d.email;
        if (d.birth_date !== undefined)
            patch.birth_date = d.birth_date;
        if (d.job_title !== undefined)
        ...
        if (d.crmv_uf !== undefined)
            patch.crmv_uf = d.crmv_uf;
        if (d.internal_notes !== undefined)
            patch.internal_notes = d.internal_notes;
        if (d.active !== undefined)
            patch.active = d.active;
        if (d.has_hub_access !== undefined)
            patch.has_hub_access = d.has_hub_access;
        if (d.hub_access_email !== undefined)
        ...
        if (d.available_days !== undefined)
            patch.available_days = d.available_days;
        if (d.work_hours !== undefined)
            patch.work_hours = d.work_hours;
        if (d.break_minutes !== undefined)
            patch.break_minutes = d.break_minutes;
        if (d.default_unit_id !== undefined) {
            if (d.default_unit_id) {
                const { data: u } = await supabase_1.supabaseAdmin.from('units').select('id, clinic_id').eq('id', d.default_unit_id).maybeSingle();
                if (!u || u.clinic_id !== clinic_id)
                    return res.status(400).json({ error: 'Unidade inválida' });
            }
            patch.default_unit_id = d.default_unit_id;
        }
        if (d.agenda_color !== undefined)
            patch.agenda_color = d.agenda_color;
...
const inviteHubStaff = async (req, res) => {
    ...
    try {
        ...
        const canStaff = await (0, authMiddleware_1.checkPermission)(invited_by, clinic_id, 'hub.staff.invite');
        const canUserInvite = await (0, authMiddleware_1.checkPermission)(invited_by, clinic_id, 'user.invite');
        ...
        if (!row.has_hub_access)
            return res.status(400).json({ error: 'Profissional sem acesso ao Hub — active «Tem acesso» primeiro' });
        const email = row.hub_access_email?.trim();
        if (!email)
            return res.status(400).json({ error: 'Defina o e-mail de acesso antes de convidar' });
        const role = row.hub_access_role;
        if (!role)
            return res.status(400).json({ error: 'Defina o perfil de acesso antes de convidar' });
        const unit_id = row.default_unit_id;
        if (!unit_id)
            return res.status(400).json({ error: 'Defina a unidade padrão antes de convidar' });
        ...
        if (existingUnitUsers?.length) {
            for (const cu of existingUnitUsers) {
                const { data: authUser } = await supabase_1.supabaseAdmin.auth.admin.getUserById(cu.user_id);
        ...
        const { data: invRows, error: invErr } = await supabase_1.supabaseAdmin
            .from('user_invitations')
            .insert([
            {
                email,
...
exports.inviteHubStaff = inviteHubStaff;
