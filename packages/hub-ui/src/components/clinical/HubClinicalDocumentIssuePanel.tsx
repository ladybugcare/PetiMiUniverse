...
import type { HubClinicalDocumentRow } from '../../api/hubClinicalApi';
...
type Props = {
  ...
  loading?: boolean;
  error?: string | null;
  document: HubClinicalDocumentRow | null;
...
export function HubClinicalDocumentIssuePanel({
  open,
  onClose,
  loading,
  error,
  ...
  publicUrl,
  contentHashShort,
  titleLoading = 'Gerando documento validável…',
  titleReady = 'Documento validável emitido',
  ...
  const link = useMemo(() => {
    ...
    return document?.validation_url ?? document?.public_url ?? '';
  }, [document, publicUrl]);
