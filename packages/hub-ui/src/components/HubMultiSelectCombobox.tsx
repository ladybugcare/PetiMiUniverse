...
export const HubMultiSelectCombobox: React.FC<HubMultiSelectComboboxProps> = ({
  ...
  const labelFor = useCallback(
    (v: string) => {
      const fromOptions = options.find((o) => o.value === v)?.label;
      if (fromOptions) return fromOptions;
      return resolveLabel ? resolveLabel(v) : v;
    },
  ...
  const showCreate =
    allowCreate &&
    !!qTrim &&
    !value.some((v) => norm(labelFor(v)) === norm(qTrim) || norm(v) === norm(qTrim)) &&
  ...
  const onCreate = () => {
    if (!qTrim || disabled) return;
    const existing = options.find((o) => norm(o.label) === norm(qTrim));
    const next = existing ? existing.value : qTrim;
            ...
            return (
              <li key={o.value} role="presentation">
                <button
                  ...
                  role="option"
                  aria-selected={isSel}
                  className={`hub-combobox__option ${isSel ? 'hub-combobox__option--selected' : ''} hub-combobox__option--no-icon`.trim()}
  ...
  return (
    <div
      ref={rootRef}
      className={`hub-combobox hub-combobox--multi ${disabled ? 'hub-combobox--disabled' : ''} ${className}`.trim()}
    >
      <button
        type="button"
        id={id}
      ...
      >
        <span className="hub-combobox__multi-tags">
          {value.length === 0 ? (
            ...
            value.map((v) => (
              <span key={v} className="hub-combobox__tag">
                <span className="hub-combobox__tag-label">{labelFor(v)}</span>
                {!disabled ? (
                  <span
                    role="button"
                    tabIndex={0}
                    className="hub-combobox__tag-remove"
