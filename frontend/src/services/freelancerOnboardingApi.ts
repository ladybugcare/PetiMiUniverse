...
export const freelancerOnboardingApi = {
   ...
   */
  completeOnboarding: async (data: CompleteOnboardingData): Promise<{ success: boolean; message: string; freelancer: any }> => {
    return apiRequest('/freelancers/onboarding/complete', {
      method: 'POST',
  ...
  /**
   * Faz upload de uma certificação
   */
  uploadCertification: async (file: File): Promise<UploadCertificationResponse> => {
    const formData = new FormData();
    formData.append('certification_file', file);
    ...
    const response = await fetch(`${API_URL}/freelancers/onboarding/upload-certification`, {
      method: 'POST',
      headers,
      body: formData,
    });
