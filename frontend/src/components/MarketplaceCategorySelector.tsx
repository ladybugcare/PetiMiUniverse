...
const MarketplaceCategorySelector: React.FC<MarketplaceCategorySelectorProps> = ({
  ...
  const verb = listingType === 'sale' ? 'vender' : 'procurar';
  const verbCapitalized = listingType === 'sale' ? 'Vender' : 'Procurar';

  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>O que você quer {verb}?</h1>
        <p style={styles.subtitle}>
          Escolha a categoria que melhor se adequa ao seu {listingType === 'sale' ? 'produto' : 'interesse'}
        </p>
      </div>
